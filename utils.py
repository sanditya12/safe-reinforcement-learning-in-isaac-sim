...
def is_circle_intersecting_with_line(circle_center, circle_radius, point1, point2):
    ...
    if distance <= circle_radius:
        ...
        # Check if the line segment lies entirely inside the circle
        if (
            (x1 - x0) ** 2 + (y1 - y0) ** 2 <= circle_radius ** 2 and
            (x2 - x0) ** 2 + (y2 - y0) ** 2 <= circle_radius ** 2
        ):
            return True
 
        # Check if the projection of the circle center onto the line segment lies within the segment
        dx = x2 - x1
        dy = y2 - y1
        ...
        if 0 <= t <= 1:
            px = x1 + t * dx
            py = y1 + t * dy
            if (px - x0) ** 2 + (py - y0) ** 2 <= circle_radius ** 2 + tolerance:
                return True
